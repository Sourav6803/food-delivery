...
const Newproduct = () => {
  ...
  const handleSubmit = async(e)=>{
    ...
    if(name && image && category && price){
      const fetchData = await fetch("http://localhost:7000/uploadProduct",{
        method : "POST",
        headers : {
    ...
    <div className='p-4'>
      <form className='m-auto w-full max-w-md  shadow flex flex-col p-3 bg-white' onSubmit={handleSubmit}>
        ...
        <div className='h-40 w-full bg-slate-200  rounded flex items-center justify-center cursor-pointer'>
        {
          ...
          <input type={"file"} accept="image/*" id="image"  onChange={handleUploadProfileImage} className="hidden"></input>
        </div>
        </label>

        <label className='my-1' htmlFor='price'>Price</label>
        <input type={"number"} className='bg-slate-200 p-1 my-1' name='price' onChange={handleonChange} value={data.price}></input>
